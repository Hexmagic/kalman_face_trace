...
def bbx_to_center(bbx):
    x_left, y_top, x_right, y_bottom = bbx
    w = x_right - x_left + 1
    h = y_bottom - y_top + 1
    xc = x_left + w / 2
    yc = y_top + h / 2
...
def get_angle_from_two_vectors(vec1, vec2):
    vec = np.dot(np.array(vec1), np.array(vec2)) / \
